...
DROP TABLE IF EXISTS trading_performance_table;
...
(
	...
	-- 承認社員IDまたは承認顧客IDのどちらかに値が入ったものを、承認済みとみなす。
	approvaled_employment_id int(4) COMMENT '承認社員IDまたは承認顧客IDのどちらかに値が入ったものを、承認済みとみなす。',
	-- 承認社員IDまたは承認顧客IDのどちらかに値が入ったものを、承認済みとみなす。
	approvaled_customer_id char(6) COMMENT '承認社員IDまたは承認顧客IDのどちらかに値が入ったものを、承認済みとみなす。',
...
(
	...
	-- 2…検品済み
	-- 3…小分け済み
	step int(1) DEFAULT 0 NOT NULL COMMENT '0…未作業
1…ピッキング済
2…検品済み
3…小分け済み',
...
(
	...
	suppliers_id int(12) NOT NULL COMMENT '法人顧客IDまたはメーカーIDが格納される。
※法人顧客フラグが建っている場合は前者、メーカーフラグが建っている場合は後者となる。',
...
CREATE TABLE trading_performance_table
(
	customer_id char(6) NOT NULL,
	generation int(2) NOT NULL,
	period char(6) NOT NULL,
	trading_performance int(12) NOT NULL,
	PRIMARY KEY (customer_id, generation, period)
) DEFAULT CHARACTER SET utf8;
...
ALTER TABLE trading_performance_table
	ADD FOREIGN KEY (customer_id, generation)
	REFERENCES corporation_customer_master (customer_id, generation)
	ON UPDATE RESTRICT
	ON DELETE RESTRICT
